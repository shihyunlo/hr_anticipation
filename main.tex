...
optimization problem characterizing the pedestrian's motion. The control $u$ takes 
in $\mathbbm{R}^2$ and is defined as the acceleration of the pedestrian, with 
position $x^p \in \mathbbm{R}^2$ and velocity $v^p \in \mathbbm{R}^2$. We take 
in discrete state space formulation, so $x_{t+1}^p = v_t^p + v_t^p dt$. Same 
is for the ''opponent's'' position $x^o$ and velocity $v^o$. We assume that, 
people plan with a receding-horizon strategy, namely to plan ahead for a fixed 
horizon from time $t$ to $t+N$, act according to $u_t$, and start over in the 
next time step $t+1$:

constraints on 
the temporal collisions with 
(adaptive) safety margins $r^{saf}$. 
...
$C_{N,to-go} = (x_N-x^G)^TP_N(x_N-x^G)$, whereas $x^G$ is the (sub)goal 
