...
When doing so, it is not sufficient to consider the paths of pedestrians as fixed.  Rather, the robot sneeds to recognize that its own behaviors can impact those of the pedestrians.
That is, the robot and pedestrians are all a part of a tightly coupled multiagent navigation problem.~\cite{trautman2010unfreezing}.  In this paper, we consider how to enable a robot to start by modeling a person's intended trajectory, and then take \emph{legible} navigation actions to optimize the expected joint efficiency of the robot's and the pedestrian's motions, after taking into account the pedestrian's reactions to the robot's motions.
