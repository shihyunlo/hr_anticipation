...
penalized in $C_{energy}$ on acceleration/velocity variation of the trajectory. 
...
navigating in a crowd. More specifically, with the aid 
...
the social cost function $C_{social}$ to include \textit{visibility, comfort}, 
...
velocity causes higher repellent forces (therefore higher cost function 
gradient). Here we refer it as social discomfort, defined as having high 
confronting velocity within the pedestrian's social space:
\begin{equation}
  C_{comf} = ((v^o)^T -v^p) \mathbbm{1}(r^{rel}<r^{soc}).
\end{equation}
This term penalizes maneuvers that turns towards the opponent agent for 
crossing. Along with visibility assessment, pedestrians who choose to pass 
...
Fig.~\ref{eq:receding_control} allows us to capture the fact that the person may 
...
people have different degrees of interest to interact with a robot, which 
largely affect their maneuvers during human-robot crossing. For 
example, in human crowd dynamics simulation, the safety margin is considered 
merely a function of crowd density and walk speed~\cite{helbing1995social}, 
but vary a lot from person to person around a robot, as observed during robot 
real-world deployment. 

To predict human-robot crossing behaviors, it then seems insufficient to adopt 
one set of parameters learnt from the lump-sum interaction data in the 
predictive model. Besides, crossing actions can be abrupt (considering 
