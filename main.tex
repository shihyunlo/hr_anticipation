...
in collision-risky situations, and to avoid
“legibly~\cite{dragan2013legibility}.
Most studies to date do not
take human-robot interaction into consideration, resulting in
unexpected human behaviors during deployment, and robot ineffective planning 
subject to large prediction errors in human responses. In this work, we 
therefore seek to model human reaction around a robot in collision-dangerous 
scenarios, with specific study on situations where path crossing happens among 
two agents. We associate different collision avoidance behaviors with people's 
underlying intentions, and propose to predict human motions based on such 
intentions. Preliminary results suggest large variations in different people's 
decisions on crossing path selection. We then also seek for a psychological model 
to explain the diverse responses, and propose to categorize people's 
strategies based on their assumptions on the robot. 
...
%\commentp{whose?} 
may have largely different dynamical performance, i.e. maximum speed and 
...
present features such as current heading deviation from the desired heading of 
both agents, their average velocity, relative distance, and the angle between 
...
Observations on past interaction is informative about the person's preference 
of path crossing decision as well. For example, we can count the number of 
passive crossing $\alpha$ and active crossing $\beta$, and pose a Beta 
function as the prior over the passive crossing decision $a^P$ as: 
$Beta(\alpha+1, \beta+1)$. However, diagnosis on past interaction is not 
necessary to be easy to acquire with limited sensing. 

We propose to iteratively learnt the parameter $w$ in this predictive model
...
such communication with human and to predict the resultant behavior change of 
...
During real-world robot deployments,
...
threat with some response time of reaction, which may be influenced by the 
opponent's behavior.
%(which we modeled in Eq.~\ref{eq:receding_control} as variable $n$)
The crossing 
decision of human is modeled to rely on assumptions about opponent behavior, so the 
...
model people to have different assumptions regarding the following items, as 
to describe the observed behavior suggested in Section~\ref{sec:pre}:
\begin{enumerate}
...
After the tests, people are asked about how they feel about navigating around 
the robot. Six people were invited in the test, with no prior experience to 
interact with the tested platform. We adopt a ROS people-tracking package from ~\cite{leigh2015person} to record pedestrian trajectories with an onboard 
Velodyne laser scanner.
%\commentp{to do what?}

%Preliminary results are shown in Fig.~\ref{fig:pre_result}. 
Based on the observed frequency of intention communication and the safety margins 
kept from the robot, we are able to informally categorize people into four groups, 
