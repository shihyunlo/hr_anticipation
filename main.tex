...
karamouzas2009predictive}, and provided insights in human collision avoidance behaviors.
...
However, to consider a \emph{robot} planning in crowds, behaviors observed 
among human pedestrians may not fully cover their dynamics with the robot. 
In particular, while deploying a robot in the real world, there has been a 
large gap between the predicted pedestrian motion and observed trajectories in 
state-of-the-art approaches~\cite{trautman2015robot,pfeiffer2016predicting}. 
None of the work in the literature has addressed human-robot dynamics in 
robot planning, which can lead to inefficient and/or unsafe robot behaviors.
...
in crowd problem by taking into account the pedestrian's reactions to the 
...
attention to scenarios with path crossing, as illustrated in Fig.~\ref{fig1}.

In such situations with potential future workspace occlusion, pedestrian path 
decisions fall into two homotopy classes: either to pass from the front of the 
confronting agent, or from behind. Such behaviors capture the dynamics of 
...
strategies. Such model helps us identify a particular person's strategy as it 
approaches the robot,
************************
and the features in the dynamics patterns of the 
person's past trajectories to distinguish 
 

to train a predictive model of pedestrian
