...
cost-to-go estimate (such as considering end-pose distance\comentp{what is "end-pose distance"?} with a large group 
of static people).
...
to the crossing decisions\commentp{say whose crossing decisions} (considering 
sudden pedestrian dynamics changes at time $n$\commentp{is that hte same n as in equation 1?  Do we really need a variable name here?}), and the 
trajectories associated with different crossing decisions\commentp{whose?} can have 
different dynamical performance.\commentp{what is dynamical performance?} Therefore, inaccurate
prediction\commentp{of what?} can negatively affect the joint path efficiency, and possibly violate the 
safety criteria. 
...
However, to identify all those variables\commentp{say what variables:  to identify appropriate values for all the variables that affect the pedestrian's motion (i.e.\ values that describe their observed motion so far)} seems computationally intractable 
...
\commentp{revision:} Even so, when moving through crowds, humans
...
system, included to characterize the crossing scenario;\commentp{how does it characterize the crossing scenario?  What does this mean?}  $\xi^p_t$ is the past 
trajectory of the pedestrian at time $t$, included to characterize the 
pedestrian's dynamic patterns\commentp{huh?}, and $f$ is a feature function.

%%%%crossing function

We propose to iteratively learn this predictive model\commentp{what does it mean to learn the model?  What parameters nened to be set, and how do you characterize success?} by collecting data from real-world human-robot interactions. Since people are also adaptive about their reaction to a robot, 
...
\commentp{do you mean:} When a pedestrian decides what path to take in
...
pedestrian, we propose a predictive model\commentp{specify what is who is predicting what} following the formulation in 
...
Such a model enables prediction\commentp{the robot to predict the pedestrian's motion, taking into account} that takes into account the 
robot's crossing action, and facilitates realistic look-ahead planning\commentp{by the robot}. 
...
the same implemented policy.\commentp{I don't understand this last sentence}
...
on a Velodyne laser scanner.\commentp{to do what?}
