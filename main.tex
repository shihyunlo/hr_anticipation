...
pedestrians have some vague estimate of 
their own velocity $v^p$, of the "opponent's" velocity $v^o$, and of the relative distance 
$r^{rel}$ between the
...
%$r^{rel} = (x^p-x^o)^T(x^p-x^o))$, where $x^p, x^o$ are the positions of the two agents
%and the location where an expected collision will happen 
...
agents and the velocity upper-bound $v_{max}$. 
\commentp{max v should be different for robots and people, right?  Specify who's max v that is.}
...
\commentp{These are still not precisely defined.}
...
\commentp{Do you mean?  $C_{social} represents the pedestrian's social interaction tendencies when navigating in a crowd, which can differ from person to person.  There are several different forms it can take, such as a social proximity model~\cite{kruse2012legible} or a social force model~\cite{helbing1995social}.}
...
decisions~\cite{helbing1995}.  This variable, reflected in ...[say where in the equations] allows us to capture the fact that the person may have decided to 
change paths before he/she actually takes the action.
}

The temporal constraint
\commentp{What temporal constraint?}
