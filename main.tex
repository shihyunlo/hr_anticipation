...
cost-to-go estimate.\commentp{What's a social cost-to-go estimate?}
...
Here we refer it\commentp{What is "it"?} as social discomfort, defined as having high 
confronting velocity\commentp{Still don't know what that means.} within the pedestrian's social space:
\begin{equation}
  C_{comf} = ((v^o)^T -v^p) \mathbbm{1}(r^{rel}<r^{soc}).
\end{equation}
This term penalizes maneuvers that turn towards the opponent agent for 
crossing.\commentp{Can we just delete "for crossing" and still have this make sense?}
Along with visibility assessment, pedestrians who choose to pass 
behind are modeled to slow down\commentp{I don't know what "are modeled to slow down" means} and stay mostly on the original route, shown 
in Fig.~\ref{fig:p1}.
...
$n$ (response time), $r^{saf}$ (safety margin), $\omega$ (navigation social preference weighting)\commentp{I don't understand what that means.  And you never actually defined $\omega$ above...}, and possibly the weighting on other cost terms\commentp{such as?}. Those psychological 
...
motion, such as speeding up and turning away, indicates the intended path 
...
the robot's motion, so as to feel \textit{safe} around a robot in a 
collision-dangerous situation. We then propose to achieve predictable and 
...
Intention communication is commonly observed in crowd navigation, to reach 
consensus on the ways the two agents are going to void. 
--------------
